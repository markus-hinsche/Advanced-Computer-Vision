...
def TEST_ENCODER_BLOCK(encoder_block):
    ...
    if (112, 112, 32) != outs[0].shape[1:]:
        return html_print(colored_string("IMPLEMENTATION IS NOT CORRECT. FIRST OUTPUT SHOULD BE FROM THE MaxPooling2d LAYER with the correct shape", "red"))

    if (224, 224, 32) != outs[1].shape[1:]:
        return html_print(colored_string("IMPLEMENTATION IS NOT CORRECT. SECOND OUTPUT SHOULD BE FROM THE Conv2d LAYER with the correct shape", "red"))
